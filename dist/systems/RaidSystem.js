...
exports.RaidSystem = void 0;
class RaidSystem {
    /**
     * 验证 Raid 目标是否合法。
     */
    validateRaidTarget(state, playerId, card, targetInstanceId) {
        if (!card.raid) {
     ...
     */
    validateRaidTargetWithRegistry(state, playerId, card, targetInstanceId, getCardData) {
        if (!card.raid) {
            return { valid: false, error: 'Card does not have Raid' };
        }
        const player = state.players[playerId];
     ...
     */
    performRaid(state, playerId, cardInstanceId, targetInstanceId, card, targetZone) {
        const changes = [];
        const events = [];
        const log = [];
        const player = state.players[playerId];
     ...
     */
    handleRaidCardLeaving(state, instanceId, destination) {
        const changes = [];
...
exports.RaidSystem = RaidSystem;
//# sourceMappingURL=RaidSystem.js.map