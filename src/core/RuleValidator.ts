...
export class RuleValidator {
  ...
  constructor(energySystem?: EnergySystem) {
    this.energySystem = energySystem || new EnergySystem();
  }
  ...
  validateAction(
    ...
    if (isCardPlayAction && options?.cardData && options?.getCardData) {
      if (!this.energySystem.canAfford(state, playerId, options.cardData, options.getCardData)) {
        const pool = this.energySystem.calculateEnergyWithRegistry(
          state, playerId, options.getCardData,
        );
        return {
          valid: false,
