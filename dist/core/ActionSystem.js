...
class ActionSystem {
    constructor(eventBus, ruleValidator, abilitySystem, energySystem, raidSystem, getCardData) {
    ...
    submitAction(state, action) {
        ...
        const validation = this.ruleValidator.validateAction(state, action.actionType, action.playerId, {
            cardData,
    ...
    // Play Character
    // ==========================================================================
    handlePlayCharacter(state, action) {
        const playerId = action.playerId;
        const cardInstanceId = action.cardInstanceId;
        const targetZone = action.targetZone || 'FrontLine';
        ...
        // 6. 检测 WhenPlayed 能力
        const triggered = this.abilitySystem.detectTriggeredAbilitiesWithRegistry(workingState, cardPlayedEvent, this.getCardData);
        ...
        if (winner) {
            workingState = { ...workingState, winner, phase: 'GameOver' };
            events.push({
                eventType: 'GameEnded',
                data: { winner },
            });
    ...
    handleUseEvent(state, action) {
        ...
        if (eventAbilities.length > 0) {
            ...
            const effectResult = this.abilitySystem.executeEffects(eventAbilities.flatMap((a) => a.effects), workingState, { ...context, sourceCard: context.sourceCard }, this.getCardData);
            workingState = (0, GameState_1.applyChanges)(workingState, effectResult.changes);
    ...
    // Activate Ability
    // ==========================================================================
    handleActivateAbility(state, action) {
        const result = this.abilitySystem.activateAbility(state, action.playerId, action.cardInstanceId, action.abilityId, this.getCardData);
        return result;
    }
    // ==========================================================================
    // Perform Raid
    // ==========================================================================
    handlePerformRaid(state, action) {
        const playerId = action.playerId;
        const cardInstanceId = action.cardInstanceId;
        ...
        // Validate target
        const targetValidation = this.raidSystem.validateRaidTargetWithRegistry(state, playerId, cardData, targetInstanceId, this.getCardData);
        if (!targetValidation.valid) {
            return { success: false, error: targetValidation.error, newState: state, events: [], log: [] };
        }
        // Execute Raid
        const raidResult = this.raidSystem.performRaid(state, playerId, cardInstanceId, targetInstanceId, cardData, 'FrontLine');
        let workingState = (0, GameState_1.applyChanges)(state, raidResult.changes);
    ...
    // ==========================================================================
    handleEndPhase(state, action, _nextPhase) {
    ...
    handleExtraDraw(state, action) {
        ...
        const newState = (0, GameState_1.applyChanges)(state, changes);
        return { success: true, newState, events: [], log: ['Extra draw: paid 1AP, drew 1 card'] };
    }
    // ==========================================================================
    // Helpers
    // ==========================================================================
    getCardByInstance(state, instanceId) {
    ...
    checkWinCondition(state) {
        for (const [playerId, playerState] of Object.entries(state.players)) {
            if (playerState.lifeArea.length === 0) {
                return (0, GameState_1.getOpponentId)(state, playerId);
            }
        }
        return null;
    }
