...
class AbilitySystem {
     ...
     */
    detectTriggeredAbilities(state, event) {
        const result = [];
     ...
     */
    detectTriggeredAbilitiesWithRegistry(state, event, getCardData) {
        const result = [];
    ...
    evaluateCondition(condition, state, context) {
        ...
        switch (type) {
            ...
            case 'IfOpponentHasCharacter': {
                const opponentId = (0, GameState_1.getOpponentId)(state, context.sourcePlayerId);
     ...
     * 返回 null 表示不可支付。
     */
    checkCosts(costs, state, context) {
        for (const cost of costs) {
            if (!this.checkSingleCost(cost, state, context))
                return false;
        }
        return true;
    }
    ...
    checkSingleCost(cost, state, context) {
        const player = state.players[context.sourcePlayerId];
    ...
    paySingleCost(cost, _state, context) {
        ...
        switch (cost.costType) {
            ...
            case 'SidelineThisCard':
                return [{
                        type: 'MOVE_CARD',
                        instanceId: context.sourceCardInstanceId,
                        from: 'FrontLine', // Will be resolved by the caller
                        to: 'Sideline',
                        playerId,
                    }];
            case 'DiscardFromHand': {
     ...
     */
    executeEffects(effects, state, context, getCardData) {
        const allChanges = [];
        const allEvents = [];
    ...
    executeSingleEffect(effect, state, context, _getCardData) {
        const playerId = context.sourcePlayerId;
        const changes = [];
        const events = [];
     ...
     */
    resolveQueue(state, queue, getCardData) {
        if (queue.length === 0) {
        ...
        for (const ability of ordered) {
            ...
            if (costChanges.length > 0) {
                workingState = (0, GameState_1.applyChanges)(workingState, costChanges);
            ...
            // 执行效果
            const effectResult = this.executeEffects(ability.abilityData.effects, workingState, context, getCardData);
     ...
     */
    activateAbility(state, playerId, cardInstanceId, abilityId, getCardData) {
    ...
    // Helpers
    // ==========================================================================
    findCardOnField(state, instanceId) {
        for (const [, player] of Object.entries(state.players)) {
