...
export interface DamageResult {
  changes: StateChange[];
  lifeCardsRevealed: CardInZone[]; // 被翻开（需要 TriggerSystem 处理）
}
...
export class DamageSystem {
   ...
   */
  calculateDamage(damageModifiers: { base?: number; multiplier?: number; bonus?: number }): number {
   ...
   * 提取卡牌的伤害修正参数。
   */
  extractDamageModifiers(abilityEffects: Array<{ effectType: string; params?: Record<string, any> }>): {
    multiplier?: number;
    bonus?: number;
  } {
    const result: { multiplier?: number; bonus?: number } = {};
