...
exports.getTurnNumber = getTurnNumber;
...
// ============================================================================
function toCardInZone(cards, ownerId) {
...
function toCardOnField(cardInZone, bp, state) {
    return {
        ...cardInZone,
        state,
        currentBP: bp,
...
function createAPCards(ownerId, count) {
    const result = [];
...
function buildInitialPlayerState(playerId, order, deck) {
    // 1. 洗牌
...
function applySingleChange(state, change) {
    switch (change.type) {
        case 'MOVE_CARD': {
        ...
        case 'DRAW_CARD': {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (!card)
                    return { success: false, state }; // 牌库空
        ...
        case 'SHUFFLE_DECK': {
            ...
            if (!player)
                return { success: false, state };
            const cards = []; // We don't have CardData refs here, just shuffle instances
            // Simply shuffle the CardInZone array
            const shuffled = (0, shuffle_1.shuffle)(player.deck);
...
// 内部辅助
// ============================================================================
function getZoneArray(player, zone) {
    switch (zone) {
 ...
 */
function recalculatePlayerCaches(state) {
    const newPlayers = {};
