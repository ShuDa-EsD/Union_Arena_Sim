...
import { GameState, GamePhase, GameEvent, StateChange } from './types';
...
export class TurnManager {
   ...
   * 自动执行阶段的入口/出口动作。
   */
  advancePhase(state: GameState): { newState: GameState; events: GameEvent[] } {
    ...
    // 执行新阶段的入口动作
    let phaseResult: { newState: GameState; events: GameEvent[] };
    ...
    // TurnStarted 事件
    if (nextPhase === 'Start') {
      const turnStartedEvent: GameEvent = {
  ...
  private executeStartPhase(state: GameState): { newState: GameState; events: GameEvent[] } {
    ...
    if (!isFirstTurn) {
      if (player.deck.length > 0) {
        ...
        workingState = { ...workingState, winner: opponentId, phase: 'GameOver' as GamePhase };
  ...
  private executeEndPhase(state: GameState): { newState: GameState; events: GameEvent[] } {
    const playerId = state.currentPlayerId;
    const player = state.players[playerId];
    const changes: StateChange[] = [];
    const events: GameEvent[] = [];
  ...
  private switchToNextPlayer(state: GameState): GameState {
    const opponentId = getOpponentId(state, state.currentPlayerId);
  ...
  private setPhase(state: GameState, phase: GamePhase): GameState {
    return { ...state, phase };
  }
  ...
  private createEvent(eventType: GameEvent['eventType'], state: GameState): GameEvent {
    return {
      eventType,
  ...
  private createPhaseEvent(state: GameState, newPhase: GamePhase, previousPhase: GamePhase): GameEvent {
    return {
