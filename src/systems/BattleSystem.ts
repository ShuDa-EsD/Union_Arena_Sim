...
import {
  GameState, StateChange, GameEvent, BattleState, BattleStep,
  CardData, CardOnField, AbilityInstance, AbilityData,
} from '../core/types';
...
export class BattleSystem {
  ...
  private attackedThisTurn: Set<string> = new Set();
  ...
  canDeclareAttack(state: GameState): boolean {
    ...
    if (!state.battleState) return true; // No active battle
  ...
  declareAttack(
    ...
    // 攻击角色 Active → Resting
    let workingState = applyChanges(state, [
    ...
    if (getCardData) {
      const triggered = this.abilitySystem.detectTriggeredAbilitiesWithRegistry(
        workingState, attackEvent, getCardData,
      );
  ...
  declareBlock(
    ...
    const defenderId = getOpponentId(state, state.currentPlayerId);
  ...
  skipBlock(
    ...
    const workingState = {
      ...state,
      battleState: { ...state.battleState, step: 'Resolving' as BattleStep },
    };
    ...
    if (bs.blocker) {
      ...
      if (attacker.currentBP >= blocker.currentBP) {
        ...
        if (getCardData) {
          ...
          if (impactDamage > 0) {
            const dmgResult = this.damageSystem.dealDamage(
              workingState,
              getOpponentId(workingState, state.currentPlayerId),
              impactDamage,
              state.currentPlayerId,
            );
            workingState = applyChanges(workingState, dmgResult.changes);
            bs.damageDealt += impactDamage;

            // Trigger 处理
            const trigResult = this.triggerSystem.processTriggers(
              workingState, dmgResult.lifeCardsRevealed,
              getOpponentId(workingState, state.currentPlayerId),
              getCardData,
            );
      ...
      // WhenNotBlocked 能力
      if (getCardData) {
        const notBlockedEvent: GameEvent = {
        ...
        const triggered = this.abilitySystem.detectTriggeredAbilitiesWithRegistry(
          workingState, notBlockedEvent, getCardData,
        );
        if (triggered.length > 0) {
          const result = this.abilitySystem.resolveQueue(workingState, triggered, getCardData);
      ...
      const dmgModifiers = getCardData
        ? this.damageSystem.extractDamageModifiers(
            getCardData(this.getCardIdFromInstance(workingState, attacker.instanceId) || '')?.abilities.flatMap(a => a.effects) || [],
          )
        : {};
      const damage = this.damageSystem.calculateDamage(dmgModifiers);

      // 造成伤害
      const targetPlayerId = getOpponentId(workingState, state.currentPlayerId);
      const dmgResult = this.damageSystem.dealDamage(
        workingState, targetPlayerId, damage, state.currentPlayerId,
      );
      workingState = applyChanges(workingState, dmgResult.changes);
      ...
      // Trigger 处理
      const trigResult = this.triggerSystem.processTriggers(
        workingState, dmgResult.lifeCardsRevealed, targetPlayerId, getCardData || (() => undefined),
      );
      workingState = trigResult.newState;
      events.push(...trigResult.events);
      log.push(...trigResult.log);
  ...
  private endBattle(
    ...
    let workingState: GameState = { ...state, battleState: { ...state.battleState!, step: 'Ending' as BattleStep } };
    ...
    if (getCardData) {
      ...
      if (attacker && isFirstAttackThisTurn) {
        ...
        const hasDoubleAttack = cardData?.abilities.some(
          (a) => a.effects.some((e) => e.effectType === 'DoubleAttack'),
        );
        if (hasDoubleAttack) {
          workingState = applyChanges(workingState, [
  ...
  private findAttacker(state: GameState, instanceId: string): CardOnField | null {
  ...
  private findBlocker(state: GameState, instanceId: string): CardOnField | null {
    return this.findAttacker(state, instanceId);
  }

  private findCharacterOwner(state: GameState, instanceId: string): string {
    for (const [playerId, player] of Object.entries(state.players)) {
      if (player.frontLine.some((c) => c.instanceId === instanceId)) return playerId;
    }
    return getOpponentId(state, state.currentPlayerId);
  }
  ...
  private hasSnipeAbility(card: CardOnField, getCardData?: (cardId: string) => CardData | undefined): boolean {
    if (!getCardData) return false;
    const data = getCardData(card.cardId);
    if (!data) return false;
    return data.abilities.some((a) =>
      a.effects.some((e) => e.effectType === 'AllowTargetCharacter' || e.effectType === 'PreventBlock'),
    );
  }

  private checkImpact(attacker: CardOnField, getCardData: (cardId: string) => CardData | undefined): number {
